import { Bell, Bookmark, Compass, Mail, PlusCircle, TrendingUp, UserIcon, Users, Home } from 'lucide-react';
            ...
            <nav className="flex-1 flex flex-col gap-1 overflow-y-auto">
                {NAV_ITEMS.map(({ label, icon: Icon }) => (
                    <button
                        ...
                        className={`flex items-center gap-4 p-3 rounded-xl font-bold text-sm transition-transform duration-200 hover:-translate-y-0.5 active:scale-95 ${activeTab === label ? 'text-emerald-700 bg-emerald-100/50' : 'text-gray-500 hover:bg-gray-100'
                            }`}
                    >
                        <Icon size={20} />
                        <span>{label}</span>
                ...
                {NAV_ITEMS_SECONDARY.map(({ label, icon: Icon }) => (
                    <button
                        ...
                        className={`flex items-center gap-4 p-3  hover:bg-gray-100 rounded-xl font-medium text-sm transition-transform duration-200 hover:-translate-y-0.5 active:scale-95${activeTab === label ? 'text-emerald-700 bg-emerald-100/50' : 'text-gray-500 hover:bg-gray-100'
                            }`}
                    >
                        <Icon size={20} />
                        <span>{label}</span>
